...
class LibrariesIOConnector(selery_utils.Connector):
    ...
    def findProject(self, platform, depName):
        ...
        project = None
        search = Search()
        results = search.project_search(filters={"keywords": depName, "manager": platform})
    ...
    def findRepository(self, project):
        ...
        repositoryData = search.repository(host="github", owner=project.owner, repo=project.name)
        ### Restore stdout
    ...
    def findProjectDependencies(self, project):
        ...
        repositoryData = search.repository_dependencies(host="github", owner=project.owner, repo=project.name)
        ### Restore stdout
        sys.stdout = sys.__stdout__
        if(repositoryData):
