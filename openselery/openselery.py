...
class OpenSelery(object):
    ...
    def gather(self):
        ...
        if self.config.include_self:
            self.logWarning("Including local project '%s'" % self.config.directory)
            ### find official repositories
            localProject = self.githubConnector.grabLocalProject(self.config.directory)
        ...
        if self.config.include_dependencies:
            self.log("Searching for dependencies of project '%s' " % self.config.directory)
            ...
            unique_dependency_dict = selery_utils.getUniqueDependencies(dependencies_json)
            for platform, depList in unique_dependency_dict.items():
                for dep in depList:
                    ...
                    print("  > %s" % ("FOUND %s" % libIoProject if libIoProject else "NOT FOUND"))
                    ### gather more information for project dependency
                    if libIoProject:
                        libIoRepository = self.librariesIoConnector.findRepository(libIoProject)
                        libIoDependencies = self.librariesIoConnector.findProjectDependencies(libIoProject)
                        print("  > %s" % [dep.project_name for dep in libIoDependencies])

                        if libIoRepository:
                            gitproject = self.githubConnector.grabRemoteProject(libIoRepository.github_id)
        ...
        for p in generalProjects:
            ...
            ### filter contributors
            depContributors = selery_utils.validateContributors(depContributors, self.config.min_contributions)
             ### safe contributor information
            generalContributors.extend(depContributors)
        print("=======================================================")
        self.logNotify("Gathered '%s' valid repositories" % len(generalProjects))
        self.logNotify("Gathered '%s' valid dependencies" % len(generalDependencies))
        self.logNotify("Gathered '%s' valid contributors" % len(generalContributors))
    ...
    def choose(self, contributors):
        ...
        recipients = random.choices(contributors, weights, k=self.config.contributor_payout_count)
        for contributor in recipients:
            print(" -- '%s': '%s' [w: %s]" % (contributor.stats.author.html_url, contributor.stats.author.email, weights[contributors.index(contributor)]))
    ...
    def payout(self, recipients):
        ...
        if self.config.dryrun:
            self.logWarning("Configuration 'dryrun' is active, so NO transaction will be executed")
        ...
        for contributor in recipients:
            if not self.config.dryrun:
                ...
                print(" -- would have been a payout of '%.10f' to '%s'" % (self.config.btc_per_transaction, contributor.stats.author.email))
